...
```bash
nextflow run mskcc/neoqual-nf -profile docker -params-file params.yaml
```
...
```bash
nextflow pull mskcc/neoqual-nf
```
...
First, go to the [mskcc/NeoQual-nf releases page](https://github.com/mskcc/NeoQual-nf/releases) and find the latest pipeline version - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`. Of course, you can switch to another version by changing the number after the `-r` flag.
